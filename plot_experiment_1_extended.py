...
def main():
    ...
    # --- パネル (A): E[L_main] ---
    ax_main.set_xlabel(r"$\lambda_1$", fontsize=20)
    ax_main.set_ylabel(r"$E[L_{\mathrm{main}}]$", fontsize=20)
    ax_main.set_title(r"(A) $E[L_{\mathrm{main}}]$", fontsize=20)
    ax_main.set_ylim(0, 70)
    ax_main.legend(fontsize=12)
    ax_main.grid(True, linestyle="--", alpha=0.3)

    # --- パネル (B): E[L_merge] ---
    ax_merge.set_xlabel(r"$\lambda_1$", fontsize=20)
    ax_merge.set_ylabel(r"$E[L_{\mathrm{merge}}]$", fontsize=20)
    ax_merge.set_title(r"(B) $E[L_{\mathrm{merge}}]$", fontsize=20)
    ax_merge.grid(True, linestyle="--", alpha=0.3)

    # --- パネル (C): P_block (対数スケール) ---
    ax_pblock.set_xlabel(r"$\lambda_1$", fontsize=20)
    ax_pblock.set_ylabel(r"$P_{\mathrm{block}}$", fontsize=20)
    ax_pblock.set_title(r"(C) Blocking Probability $P_{\mathrm{block}}$", fontsize=20)
    ax_pblock.set_yscale("log")
    ax_pblock.grid(True, linestyle="--", alpha=0.3)

    # --- パネル (D): 合計スループット ---
    ax_tput.set_xlabel(r"$\lambda_1$", fontsize=20)
    ax_tput.set_ylabel(r"$\lambda_1 + \lambda_2(1 - P_{\mathrm{block}})$", fontsize=20)
    ax_tput.set_title(r"(D) Total Throughput", fontsize=20)
    ax_tput.grid(True, linestyle="--", alpha=0.3)

    # --- 目盛りテキストのフォントサイズ ---
    for ax in (ax_main, ax_merge, ax_pblock, ax_tput):
