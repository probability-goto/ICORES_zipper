...
# パラメータ設定
# ---------------------------------------------------------------------------
MU1_RAND   = 40    # ランダムモード本線サービス率（mu1_zip=30 より高速 → ハイブリッドの優位性の源）
MU2_RAND   = 30    # ランダムモード合流車線サービス率
MU1_ZIP    = 30    # ジッパーモード本線サービス率
MU2_ZIP    = 20    # ジッパーモード合流車線サービス率
LAM2       = 10
P          = 0.5
...
# P_block = 5% 等高線が K=5 で m=0(λ1≈10) vs m=0.6K(λ1≈22-23) の押し上げを示す
# 10%以上の等高線は K=5..30 の安定域内に存在しないため描画されない
