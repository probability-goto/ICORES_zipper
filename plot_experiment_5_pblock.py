...
出力: fig_experiment_5_pblock.png
      (--high-only 指定時は High Load のグラフのみを fig_experiment_5_pblock_high.png に出力)
"""
...
def main(high_only: bool = False):
    n_m = len(M_VALUES)
    n_p = len(P_VALUES)

    # high_only 指定時は High Load (LOAD_NAMES[2]) のみを対象とする
    load_indices = [2] if high_only else [0, 1, 2]
    n_loads = len(load_indices)
    ...
    # pblock[m_idx, li, p_idx] (li は load_indices 内でのインデックス)
    pblock = np.full((n_m, n_loads, n_p), np.nan)

    total_pts = n_m * n_loads * n_p
    print(f"\nStep 2: Computing P_block ({total_pts} total points) ...")

    with tqdm(total=total_pts, desc="P_block", unit="pt") as pbar:
        for mi, m in enumerate(M_VALUES):
            for pi_idx, p in enumerate(P_VALUES):
                for li, load_idx in enumerate(load_indices):
                    load_level = LOAD_NAMES[load_idx]
    ...
    plt.tight_layout()
    output_path = "fig_experiment_5_pblock_high.png" if high_only else "fig_experiment_5_pblock.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="High Load のグラフのみを出力する (他の負荷水準は計算・出力しない)",
    )
    args = parser.parse_args()
    main(high_only=args.high_only)
